...
# Apparently, -1 means unlimited, but we will start with a small result set.
# The entire set might never be fetch()'ed after all!
REPLY_SIZE <- 100

# .mapiRead and .mapiWrite implement MonetDB's MAPI protocol.
# It works as follows:
# On top of the socket stream, blocks are being sent.
# Each block has a two-byte header.
# MAPI protocol messages are sent as one or more blocks in both directions.
# The block header contains two pieces of information:
#   1) the amount of bytes in the block,
...
.mapiRequest <- function(conObj, msg, async = FALSE) {
  # call finalizers on disused objects.
  # At least avoids concurrent access to socket.
  # gc()

  if (!identical(class(conObj)[[1]], "MonetDBConnection")) {
    stop("I can only be called with a MonetDBConnection as parameter,
         not a socket.")
  }

  # Ugly effect of finalizers, sometimes, single-threaded R can get concurrent
  #   calls to this method.
  if (conObj@connenv$lock > 0) {
    if (async) {
      if (getOption("monetdb.debug.mapi", F)) {
        message(
          "II: Attempted parallel access to socket. ",
          "Deferred query '", msg, "'"
        )
      }
      ...
      stop(paste(
        "II: Attempted parallel access to socket.",
        "Use only dbSendUpdateAsync() from finalizers."
      ))
    }
  }

  # add exit handler that will clean up the socket and release the lock.
  # just in case someone (Anthony!) uses ESC or CTRL-C while running some long
  # running query.
  on.exit(.mapiCleanup(conObj), add = TRUE)

  # prevent other calls to .mapiRequest while we are doing something on this
  # connection.
  ...
  while (length(conObj@connenv$deferred) > 0) {
    ...
    if (dresp$type == MSG_MESSAGE) {
      ...
      warning(paste(
        "II: Failed to execute deferred statement '", dmesg,
        "'. Server said: '", dresp$message, "'"
      ))
    }
  }
  # release lock
...
.mapiRead <- function(con) {
  ...
  resp <- list()
  repeat {
    unpacked <- readBin(con, "integer",
      n = 1, size = 2, signed = FALSE,
      endian = "little"
    )

    if (length(unpacked) == 0) {
      stop(paste(
        "Empty response from MonetDB server, probably a timeout.",
        "You can increase the time to wait for responses with",
        "the 'timeout' parameter to 'dbConnect()'."
      ))
    }
  ...
  if (getOption("monetdb.debug.mapi", F)) {
    message("RX: '", substring(paste0(resp, collapse = ""), 1, 200))
  }
...
.mapiParseResponse <- function(response) {
  ...
  if (resKey == MSG_QUERY) {
    ...
    if (typeKey == Q_TABLE || typeKey == Q_PREPARE) {
      ...
      if (getOption("monetdb.debug.query", F)) {
        message(
          "QQ: Query result for query ", header$id, " with ",
          .mapiLongInt(header$rows), " rows and ", header$cols, " cols, ",
          header$index, " rows."
        )
      }
    ...
    if (typeKey == Q_BLOCK) {
      ...
      if (getOption("monetdb.debug.query", F)) {
        message(
          "QQ: Continuation for query ", header$id, " with ", header$rows,
          " rows and ", header$cols, " cols, index ", header$inde, "."
        )
      }
    ...
    if (typeKey == Q_TRANSACTION) {
      env$type <- Q_UPDATE
      # no need to check the returned values, as there is none.
      # If we get no error, all is well.
      return(env)
    }
  }
}
...
.mapiParseTableHeader <- function(line) {
  first <- strsplit(substr(line, 3, nchar(line)), " #",
...
# Authenticates the client against the MonetDB server. This is the first thing
# that happens on each connection. The server starts by sending a challenge, to
# which we respond by hashing our login information using the server-requested
# hashing algorithm and its salt.
.mapiAuthenticate <- function(con, dbname, user = "monetdb",
                              password = "monetdb", endhashfunc = "sha512",
                              language = "sql") {
  ...
  if (!(endhashfunc %in% HASH_ALGOS)) {
    stop(
      "Server-requested end hash function ", endhashfunc, " is not available"
    )
  }

  if (!(pwhashfunc %in% HASH_ALGOS)) {
    stop(
      "Server-requested password hash function ", pwhashfunc,
      " is not available"
    )
  }
  ...
  hashsum <- digest(paste0(
    digest(password, algo = pwhashfunc, serialize = FALSE), salt
  ),
  ...
  authString <- paste0(
    "LIT:", user, ":{", toupper(endhashfunc), "}", hashsum,
    ":", language, ":", dbname, ":"
  )
  ...
  if (respKey != MSG_PROMPT) {
    ...
    if (getOption("monetdb.debug.mapi", F)) {
      message("II: Authentication successful.")
    }
